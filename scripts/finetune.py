...
from src.model.submcl import SubMCLPredictor as SubMCL
...
def parse_args():
    parser = argparse.ArgumentParser(description="Arguments for training SubMCL")
...
def get_predictor(d_input_feats, n_tasks, n_layers, predictor_drop, device, d_hidden_feats=None, rank=None,
                  attn_dim=None, num_heads=None):  ####
    if n_layers == 1:
        ...
        for _ in range(n_layers - 2):
            predictor.append(LowRankLinear(d_hidden_feats, d_hidden_feats, rank))  ####
            predictor.append(MultiHeadAttention(d_hidden_feats, num_heads, dropout=predictor_drop))
            predictor.append(nn.Dropout(predictor_drop))
            predictor.append(nn.GELU())
...
def finetune(args):
    ...
    train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=args.n_threads,
                              worker_init_fn=seed_worker, generator=g, drop_last=True, collate_fn=collator)
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False, num_workers=args.n_threads,
                            worker_init_fn=seed_worker, generator=g, drop_last=False, collate_fn=collator)
    test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False, num_workers=args.n_threads,
                             worker_init_fn=seed_worker, generator=g, drop_last=False, collate_fn=collator)
    ...
    model.predictor = get_predictor(d_input_feats=config['d_g_feats'] * 3, n_tasks=train_dataset.n_tasks, n_layers=2,
                                    predictor_drop=args.dropout, device=device, d_hidden_feats=128, rank=8,
