...
if __name__ == '__main__':
    ...
    reg_evaluator = Evaluator("chembl", reg_metric, train_dataset.d_mds)
    clf_evaluator = Evaluator("chembl", clf_metric, train_dataset.d_fps)
    result_tracker = Result_Tracker(reg_metric)
    if local_rank == 0:
        summary_writer = SummaryWriter(f"tensorboard/pretrain-{args.config}", )
    else:
        summary_writer = None
    trainer = Trainer(args, optimizer, lr_scheduler, reg_loss_fn, clf_loss_fn, sl_loss_fn, reg_evaluator, clf_evaluator,
                      result_tracker, summary_writer, device=device, local_rank=local_rank)
