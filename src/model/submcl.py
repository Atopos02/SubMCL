...
class SubMCLPredictor(nn.Module):
    def __init__(self,
        ...
        super(SubMCLPredictor, self).__init__()
        self.d_g_feats = d_g_feats
        ...
        self.BiMap = nn.Linear(self.d_g_feats, self.d_g_feats // 2, bias=False)  # 假设将维度减半
        # self.second_order_projection = nn.Linear((d_g_feats//2)*(d_g_feats//2), d_g_feats)
        self.apply(lambda module: init_params(module))

    def second_order_pooling(self, node_embeddings):
        cur_node_embeddings = self.BiMap(node_embeddings)
        cur_graph_embeddings = torch.matmul(cur_node_embeddings.t(), cur_node_embeddings)
        dense_dim = cur_graph_embeddings.shape[0] * cur_graph_embeddings.shape[1]
        return cur_graph_embeddings.view(dense_dim)
    ...
    def forward(self, g, fp, md):
        ...
        pooled_triplet_h = self.second_order_pooling(triplet_h) 
    ...
    def forward_tune(self, g, fp, md):
        ...
        pooled_triplet_h = self.second_order_pooling(triplet_h)  # (d_g_feats//2,d_g_feats//2)
        # Readout
        ...
        # Predict
        return self.predictor(g_feats), triplet_h, pooled_triplet_h
